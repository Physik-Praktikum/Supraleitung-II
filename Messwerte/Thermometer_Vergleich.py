...
Ts_alt, Rs_alt = np.loadtxt("alte_Kalibrierung_Ge_Thermometer.dat",skiprows=1,unpack=True)
Ts_Germ, dTs_Germ = Germanium(Rs_alt)

fig = plt.figure()
ax = fig.add_subplot(111)
ax.plot(Ts_Germ,Ts_Germ-Ts_alt,"o",markersize=4,color="blue",label=r"$T_{\mathrm{fit}} - T_{\mathrm{alt}}$")
#ax.set_ylim(-100,100)
ax.set_xlabel(r"$T_{\mathrm{fit}}$/K",fontsize=14)
ax.set_ylabel(r"$T_{\mathrm{fit}} - T_{\mathrm{alt}}$/K",fontsize=14)
